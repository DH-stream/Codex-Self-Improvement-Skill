...
4. Before executing a multi-task plan, review task dependencies, file ownership, shared mutable state, and risk. Keep the primary implementation inline in the main agent; after each completed logical snapshot, launch an independent non-editing reviewer while main work continues when safe, and use a separate isolated fixer for actionable review findings.
5. Treat `tests/baseline-observations.md` and additional `tests/baseline-*.md` files as RED evidence and add/update a relevant pressure scenario before behavior changes.
...
17. Review actual files and the complete diff against current `main`, not only plans, reports, or PR descriptions.
18. Review subagents report findings only; a fresh fixer subagent owns review-driven edits. Concurrent write agents must have disjoint ownership.
19. After all implementation/review/fix pipelines return, the main agent performs the final integration review and fresh full verification itself.
20. Update public `memory/UPDATE_LOG.md` for material engine, universal-memory, installer, or schema changes.
21. After actual memory/skill writes, report only changed filename(s) and any confirmed draft PR reference unless the user asks for the lesson.
